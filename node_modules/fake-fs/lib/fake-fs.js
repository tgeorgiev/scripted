...
function Fs (resolveToFakeRoot) {
    this.root = new stat.Dir
    this.resolveToFakeRoot = resolveToFakeRoot;
}

Fs.prototype._resolve = function (p) {
    var resolvedPath;
    if (this.resolveToFakeRoot || p == '/') {
        if (p == ".") {
            resolvedPath = "/";
        }
        else {
            ...
            if (resolvedPath.indexOf("..") != -1) {
                 throw new Error('relative path .. is not supported when resolveToFakeRoot is enabled' );
            }
    ...
    return resolvedPath.replace(/\\/g, '/') // Windows support
}
...
Fs.prototype._add = function (path, item) {
    var resolvedPath = this._resolve(path);
    var segs = resolvedPath == '/'
        ? []
        : resolvedPath.split('/').slice(1)
